...
export class CreateInitialSchema1702000001 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'role', type: 'enum', enum: ['admin', 'user', 'guest'], default: "'user'" },
    ...
    await queryRunner.createIndex('users', new TableIndex({ columnNames: ['username'] }));
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'context_data', type: 'jsonb', isNullable: true, comment: 'Conversation context and settings' },
    ...
    await queryRunner.createIndex('conversations', new TableIndex({ columnNames: ['user_id', 'created_at'] }));
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'sender_type', type: 'enum', enum: ['user', 'assistant'], isNullable: false },
    ...
    await queryRunner.createIndex('messages', new TableIndex({ columnNames: ['conversation_id', 'created_at'] }));
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'importance_score', type: 'float', default: 0.5 },
    ...
    await queryRunner.createForeignKey('memory_contexts', new TableForeignKey({
      columnNames: ['user_id'],
      referencedColumnNames: ['id'],
      referencedTableName: 'users',
      onDelete: 'CASCADE',
    }));
    await queryRunner.createIndex('memory_contexts', new TableIndex({ columnNames: ['user_id', 'key'], isUnique: true }));
    ...
    await queryRunner.createIndex('audit_logs', new TableIndex({ columnNames: ['actor_id', 'created_at'] }));
    await queryRunner.createIndex('audit_logs', new TableIndex({ columnNames: ['resource_type', 'resource_id'] }));
